    ...
    try {
      ...
      reportDiv.innerHTML = `
        <div style="border-bottom:3px solid #0d9488;padding-bottom:16px;margin-bottom:24px;">
          <h1 style="font-size:22px;font-weight:700;color:#0d9488;margin:0 0 4px 0;">MedMink — Clinical Case Report</h1>
        ...
        <div style="border-top:1px solid #e5e7eb;padding-top:12px;color:#999;font-size:10px;">
          <p style="margin:0;">Disclaimer: This report is generated by MedMink for educational purposes only. It does not constitute medical advice. Clinical decisions should be made by qualified healthcare professionals based on individual patient assessment.</p>
      ...
      const filename = `medmink-report-${data.parsedCase.case_category || 'case'}-${new Date().toISOString().slice(0, 10)}.pdf`;
      pdf.save(filename);
    } catch (err) {
