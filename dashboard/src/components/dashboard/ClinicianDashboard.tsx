...
import type { PostVisitMessage, VitalAlert } from '@/types/postvisit';
...
export function ClinicianDashboard() {
  ...
  const [patientMessages, setPatientMessages] = useState<PostVisitMessage[]>([]);
  const [vitalAlerts, setVitalAlerts] = useState<(VitalAlert & { patientName?: string; patientId?: string })[]>([]);
  ...
  // Fetch patient messages from all released summaries
  useEffect(() => {
    const summaries = getReleasedSummaries().filter(s => s.status === 'released');
    if (summaries.length === 0) return;

    const fetchAll = async () => {
      const allMessages: PostVisitMessage[] = [];
      const allAlerts: (VitalAlert & { patientName?: string; patientId?: string })[] = [];

      await Promise.allSettled(
        summaries.map(async (s) => {
          // Fetch messages
          try {
            const msgResp = await fetch(`/api/postvisit/${s.id}/messages`);
            if (msgResp.ok) {
              const data = await msgResp.json();
              const msgs = (data.messages || []) as PostVisitMessage[];
              allMessages.push(...msgs);
            }
          } catch { /* skip */ }

          // Fetch vital alerts if patient has an ID
          if (s.patientId) {
            try {
              const vitResp = await fetch(`/api/postvisit/vitals/${s.patientId}/analyze`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: '', summary: { diagnosis: s.diagnosis }, patient_id: s.patientId }),
              });
              if (vitResp.ok) {
                const data = await vitResp.json();
                const alerts = (data.alerts || []) as VitalAlert[];
                allAlerts.push(...alerts.map(a => ({ ...a, patientName: s.diagnosis, patientId: s.patientId })));
              }
            } catch { /* skip */ }
          }
        })
      ...
      setPatientMessages(allMessages);
      setVitalAlerts(allAlerts);
    };

    fetchAll();
  }, []);

  const handleSendReply = useCallback(async (messageId: string, summaryId: string, content: string) => {
    const resp = await fetch(`/api/postvisit/${summaryId}/messages`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content, sender: 'clinician' }),
    });
    if (resp.ok) {
      const msg = await resp.json();
      setPatientMessages(prev => prev.map(m => m.id === messageId ? { ...m, status: 'replied' as const } : m).concat(msg));
    }
  }, []);

  const handleGenerateDraft = useCallback(async (messageId: string, summaryId: string): Promise<string> => {
    try {
      const resp = await fetch(`/api/postvisit/${summaryId}/messages/${messageId}/draft`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
    ...
    <div className="p-5 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              <Link href="/case?new=true" className="group">
                <div className="relative rounded-xl border border-border bg-card p-4 hover:border-primary/30 hover:shadow-sm transition-all overflow-hidden">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">New Case</p>
                      <p className="text-[11px] text-muted-foreground/60 mt-0.5">Start case analysis</p>
                    </div>
                  </div>
                  <ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/20 group-hover:text-primary/40 group-hover:translate-x-0.5 transition-all" />
                </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Vital Alerts */}
          <VitalAlertsFeed alerts={vitalAlerts} />
